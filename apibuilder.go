...
type Config struct {
	BasePath       *url.URL
	PrivateToken   string
	JsonSchemePath string "clients/"
	JsonScheme     string "index.json"
}
...
func NewApi(config *Config) (*Api) {
	...
	fileContent, err := ioutil.ReadFile(config.JsonSchemePath+config.JsonScheme);
	...
	for _, filename := range schema.Includes {
		fileContent, err := ioutil.ReadFile(config.JsonSchemePath+filename);

		if err != nil {
			log.Println("Error read", err.Error())
		}

		 json.Unmarshal(fileContent, &schema)
	}
